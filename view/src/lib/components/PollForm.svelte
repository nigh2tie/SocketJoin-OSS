...
<script>
    ...
    async function createPoll() {
        if (!newPollTitle || newPollOptions.some(o => !o.label)) return;

        const isQuiz = newPollType === 'quiz';
...
<div class={`bg-white p-6 rounded-xl shadow-sm border transition-all duration-300 ${newPollType === 'quiz' ? 'border-purple-200 bg-purple-50/30' : 'border-gray-200'}`}>
    <div class="flex items-center justify-between mb-4 border-b pb-2">
        ...
        <div class="flex bg-gray-100 p-1 rounded-lg">
            ...
            <button
                class={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${newPollType === 'quiz' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
    ...
    <div class="space-y-4">
        <div>
            <label for="new-poll-title" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
                {newPollType === 'quiz' ? 'クイズの問題文' : '質問タイトル'}
            </label>
            <input id="new-poll-title" type="text" bind:value={newPollTitle} placeholder={newPollType === 'quiz' ? '例: 日本で一番高い山は？' : '例: 本日の満足度は？'} class="w-full p-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
        </div>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <button
                on:click={createPoll}
                class={`flex-1 py-2.5 text-white font-bold rounded-lg transition text-sm ${newPollType === 'quiz' ? 'bg-purple-600 hover:bg-purple-700 shadow-purple-200 shadow-lg' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200 shadow-lg'}`}
            >
                {newPollType === 'quiz' ? 'クイズを作成' : '投票を作成'}
            </button>
        </div>
    </div>
</div>
